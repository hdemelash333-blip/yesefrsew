...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_item'])) {
    ...
    if ($id) {
        $stmt = $pdo->prepare("UPDATE interviews SET title=?, slug=?, artist_name=?, excerpt=?, content=?, status=? WHERE id=?");
        $stmt->execute([$title, $slug, $artist_name, $excerpt, $content, $status, $id]);
    } else {
        $stmt = $pdo->prepare("INSERT INTO interviews (title, slug, artist_name, excerpt, content, status) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$title, $slug, $artist_name, $excerpt, $content, $status]);
    }
    redirect('manage_interviews.php');
}
...
$items = $pdo->query("SELECT * FROM interviews ORDER BY created_at DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Manage Interviews | YESEFERSEW Admin</title>
...
<body>
    <div class="sidebar">
        ...
        <ul>
            ...
            <li><a href="manage_interviews.php" class="active"><i class="fas fa-microphone-alt"></i> Interviews</a></li>
    ...
    <div class="main-content">
        <h1>Manage Artist Interviews</h1>
        <button class="btn btn-add" onclick="showModal()">Add New Interview</button>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <a href="?delete=<?php echo $item['id']; ?>" class="btn btn-delete" onclick="return confirm('Delete this interview?')">Delete</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
    ...
    <div id="itemModal" class="modal">
        <div class="modal-content">
            <h2 id="modalTitle">Add New Interview</h2>
            <form method="POST">
                <input type="hidden" name="id" id="itemId">
                <label>Title</label>
                <input type="text" name="title" id="itemTitle" required>
                <label>Artist Name</label>
                <input type="text" name="artist_name" id="itemArtist" required>
                <label>Excerpt</label>
    ...
    <script>
        function showModal() {
            ...
            document.getElementById('modalTitle').innerText = 'Add New Interview';
        ...
        function editItem(item) {
            ...
            document.getElementById('itemArtist').value = item.artist_name;
