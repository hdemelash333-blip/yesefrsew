...
// Handle Add/Edit
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_essay'])) {
    ...
    if ($id) {
        $stmt = $pdo->prepare("UPDATE essays SET title=?, slug=?, excerpt=?, content=?, author=?, status=? WHERE id=?");
        $stmt->execute([$title, $slug, $excerpt, $content, $author, $status, $id]);
    } else {
        $stmt = $pdo->prepare("INSERT INTO essays (title, slug, excerpt, content, author, status) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$title, $slug, $excerpt, $content, $author, $status]);
    }
    redirect('manage_essays.php');
}

// Handle Delete
if (isset($_GET['delete'])) {
    $stmt = $pdo->prepare("DELETE FROM essays WHERE id=?");
    $stmt->execute([$_GET['delete']]);
    redirect('manage_essays.php');
}

$essays = $pdo->query("SELECT * FROM essays ORDER BY created_at DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Manage Essays | YESEFERSEW Admin</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        /* Shared CSS would be better in a file, but keeping it simple for now */
...
<body>
    <div class="sidebar">
        ...
        <ul>
            ...
            <li><a href="manage_essays.php" class="active"><i class="fas fa-feather-alt"></i> Essays</a></li>
    ...
    <div class="main-content">
        <h1>Manage Critical Essays</h1>
        <button class="btn btn-add" onclick="showModal()">Add New Essay</button>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo $essay['author']; ?></td>
                    <td><?php echo $essay['status']; ?></td>
                    <td><?php echo date('Y-m-d', strtotime($essay['created_at'])); ?></td>
                    <td>
                        <button class="btn btn-edit" onclick='editEssay(<?php echo htmlspecialchars(json_encode($essay), ENT_QUOTES, "UTF-8"); ?>)'>Edit</button>
                        <a href="?delete=<?php echo $essay['id']; ?>" class="btn btn-delete" onclick="return confirm('Delete this essay?')">Delete</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
    ...
    <div id="essayModal" class="modal">
        <div class="modal-content">
            <h2 id="modalTitle">Add New Essay</h2>
            <form method="POST">
                <input type="hidden" name="id" id="essayId">
                <label>Title</label>
                <input type="text" name="title" id="essayTitle" required>
                <label>Author</label>
                <input type="text" name="author" id="essayAuthor">
                <label>Excerpt</label>
                <textarea name="excerpt" id="essayExcerpt" rows="3"></textarea>
                <label>Content</label>
                <textarea name="content" id="essayContent" rows="10"></textarea>
                <label>Status</label>
                <select name="status" id="essayStatus">
                    <option value="draft">Draft</option>
                    <option value="published">Published</option>
                </select>
                <button type="submit" name="save_essay" class="btn btn-add">Save Essay</button>
    ...
    <script>
        function showModal() {
            ...
            document.getElementById('modalTitle').innerText = 'Add New Essay';
            document.getElementById('essayId').value = '';
            document.getElementById('essayTitle').value = '';
            document.getElementById('essayAuthor').value = '';
            document.getElementById('essayExcerpt').value = '';
            document.getElementById('essayContent').value = '';
        }
        function hideModal() { document.getElementById('essayModal').style.display = 'none'; }
        function editEssay(essay) {
            showModal();
            document.getElementById('modalTitle').innerText = 'Edit Essay';
            document.getElementById('essayId').value = essay.id;
            ...
            document.getElementById('essayStatus').value = essay.status;
        }
    </script>
</body>
</html>
