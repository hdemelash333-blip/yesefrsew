...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_item'])) {
    ...
    $slug = get_slug($title);
    $curator_name = sanitize($_POST['curator_name']);
    $excerpt = sanitize($_POST['excerpt']);
    $content = $_POST['content'];
    $status = $_POST['status'];

    if ($id) {
        $stmt = $pdo->prepare("UPDATE curatorial_notes SET title=?, slug=?, curator_name=?, excerpt=?, content=?, status=? WHERE id=?");
        $stmt->execute([$title, $slug, $curator_name, $excerpt, $content, $status, $id]);
    } else {
        $stmt = $pdo->prepare("INSERT INTO curatorial_notes (title, slug, curator_name, excerpt, content, status) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$title, $slug, $curator_name, $excerpt, $content, $status]);
    }
    redirect('manage_curatorial.php');
}

if (isset($_GET['delete'])) {
    $stmt = $pdo->prepare("DELETE FROM curatorial_notes WHERE id=?");
    $stmt->execute([$_GET['delete']]);
    redirect('manage_curatorial.php');
}

$items = $pdo->query("SELECT * FROM curatorial_notes ORDER BY created_at DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Manage Curatorial | YESEFERSEW Admin</title>
...
<body>
    <div class="sidebar">
        ...
        <ul>
            ...
            <li><a href="manage_curatorial.php" class="active"><i class="fas fa-chalkboard-teacher"></i> Curatorial</a></li>
    ...
    <div class="main-content">
        <h1>Manage Curatorial Notes</h1>
        <button class="btn btn-add" onclick="showModal()">Add New Note</button>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo $item['curator_name']; ?></td>
                    <td><?php echo $item['status']; ?></td>
                    <td>
                        <button class="btn btn-edit" onclick='editItem(<?php echo htmlspecialchars(json_encode($item), ENT_QUOTES, "UTF-8"); ?>)'>Edit</button>
                        <a href="?delete=<?php echo $item['id']; ?>" class="btn btn-delete" onclick="return confirm('Delete this note?')">Delete</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
    ...
    <div id="itemModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <input type="text" name="curator_name" id="itemCurator">
                <label>Excerpt</label>
                <textarea name="excerpt" id="itemExcerpt" rows="3"></textarea>
                <label>Content</label>
                <textarea name="content" id="itemContent" rows="10"></textarea>
                <label>Status</label>
    ...
    <script>
        function showModal() {
            ...
            document.getElementById('modalTitle').innerText = 'Add New Note';
            document.getElementById('itemId').value = '';
            document.getElementById('itemTitle').value = '';
            document.getElementById('itemCurator').value = '';
        ...
        function editItem(item) {
            ...
            document.getElementById('itemCurator').value = item.curator_name;
